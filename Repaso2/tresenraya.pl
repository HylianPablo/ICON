...
amenaza([X,Y,Z],B,R):-vacio(X,B),cruz(Y,B),cruz(Z,B),R is X,!.
amenaza([X,Y,Z],B,R):-cruz(X,B),vacio(Y,B),cruz(Z,B),R is Y,!.
amenaza([X,Y,Z],B,R):-cruz(X,B),cruz(Y,B),vacio(Z,B),R is Z,!.
...
movimiento_forzoso(Tablero,Casilla):-enlinea(X,Y,Z),amenaza([X,Y,Z],Tablero,Casilla),!.
