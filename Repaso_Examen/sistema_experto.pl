...
ladosIguales(X):-ask(ladosIguales,'Cuantos lados iguales?',X).
anguloRecto(X):-ask(anguloRecto,'Angulos rectos?(si/no)',X).
ladosParalelos(X):-ask(ladosParalelos,'Lados paralelos? (0/2/4)',X).
